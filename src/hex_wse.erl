...
%% session enter here 
%% Ws is the Web socket, Where is the web page id where to draw 
run(Ws, Where) ->
    io:format("Run ws=~p where=~p\n", [Ws, Where]),
    hex_wse_server:new_session(Ws, Where).
...
add_event(Flags, Signal, Cb) ->
    hex_wse_server:add_event(Flags, Signal, Cb).
...
%%
mod_event(Ref, Flags) ->
    hex_wse_server:mod_event(Ref, Flags).

%%
%%  del_event(Ref::reference()) ->
%%     ok.
del_event(Ref) ->
    hex_wse_server:del_event(Ref).
...
%%
output(Flags, Env) ->
    hex_wse_server:output_event(Flags, Env).

%%
%% init_event(in | out, Flags::[{atom(),term()}])
%%
init_event(Dir, Flags) ->
    hex_wse_server:init_event(Dir, Flags).
